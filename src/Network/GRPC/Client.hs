...
waitReply :: (Service s, HasMethod s meth) => RPC s meth -> Decoding -> Http2Stream -> IncomingFlowControl -> ClientIO (RawReply (MethodOutput s meth))
...
newtype RPCCall s (meth ::Symbol) a = RPCCall {
...
rpcFromCall :: RPCCall s meth a -> RPC s meth
rpcFromCall _ = RPC

-- | Main handler to perform gRPC calls to a service.
open :: (Service s, HasMethod s meth)
     => Http2Client
...
streamReply
  :: (Service s, HasMethod s meth, MethodStreamingType s meth ~ 'ServerStreaming)
  => RPC s meth
  -- ^ RPC to call.
  -> a
  -- ^ An initial state.
  -> MethodInput s meth
  -- ^ The input.
  -> (a -> HeaderList -> MethodOutput s meth -> ClientIO a)
  -- ^ A state-passing handler that is called with the message read.
  -> RPCCall s meth (a, HeaderList, HeaderList)
...
streamRequest
  :: (Service s, HasMethod s meth, MethodStreamingType s meth ~ 'ClientStreaming)
  => RPC s meth
  -- ^ RPC to call.
  -> a
  -- ^ An initial state.
  -> (a -> ClientIO (a, Either StreamDone (CompressMode, MethodInput s meth)))
  -- ^ A state-passing action to retrieve the next message to send to the server.
  -> RPCCall s meth (a, RawReply (MethodOutput s meth))
...
singleRequest
  ...
  -> MethodInput s meth
  -- ^ RPC's input.
  -> RPCCall s meth (RawReply (MethodOutput s meth))
...
type HandleMessageStep s meth a = HeaderList -> a -> MethodOutput s meth -> ClientIO a
-- | Handler for received trailers.
type HandleTrailersStep a = HeaderList -> a -> HeaderList -> ClientIO a

data BiDiStep s meth a =
    Abort
  -- ^ Finalize and return the current state.
  | SendInput !CompressMode !(MethodInput s meth)
  -- ^ Sends a single message.
  | WaitOutput (HandleMessageStep s meth a) (HandleTrailersStep a)
...
steppedBiDiStream
  :: (Service s, HasMethod s meth, MethodStreamingType s meth ~ 'BiDiStreaming)
  => RPC s meth
  -- ^ RPC to call.
  -> a
  -- ^ An initial state.
  -> RunBiDiStep s meth a
  -- ^ The program.
  -> RPCCall s meth a
...
data IncomingEvent s meth a =
    Headers HeaderList
  -- ^ The server sent some initial metadata with the headers.
  | RecvMessage (MethodOutput s meth)
...
data OutgoingEvent s meth b =
    Finalize
  -- ^ The client is done with the RPC (the loop stops).
  | SendMessage CompressMode (MethodInput s meth)
